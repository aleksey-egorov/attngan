...
from codebase.global_attention import GlobalAttentionGeneral as ATT_NET
...
class CNN_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        #x = F.interpolate(input=x, size=(299, 299), mode='bilinear', align_corners=True) # prev nn.Upsample
        # 299 x 299 x 3
...
class G_FC_Net(nn.Module):
    '''Generator - fully connected net'''

    def __init__(self):
        super(G_FC_Net, self).__init__()
        ngf = cfg.GAN.GF_DIM
...
# For 64 x 64 images
class D_Net_64(nn.Module):
    def __init__(self, b_jcu=True):
        super(D_Net_64, self).__init__()
        ndf = cfg.GAN.DF_DIM
        nef = cfg.TEXT.EMBEDDING_DIM
